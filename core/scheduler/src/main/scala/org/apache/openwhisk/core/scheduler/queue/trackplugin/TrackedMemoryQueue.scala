...
case object TrackedRunning extends MemoryQueueState  //  Running state with scheduling decided by the QueueSupervisor

//  State placed to prevent return to Running State during imposed idle conditions
case object TrackedIdle extends MemoryQueueState
...
case class EnableTrackedRun( qsv: QueueSupervisor ) extends RequiredAction
...
class TrackedMemoryQueue(
  ...
  private implicit val timeout: util.Timeout = Timeout(5.seconds)
  ...
  //  state similar to the Running but allowing the control of the incoming Activations/Containers by a dedicated component
  when( TrackedRunning, stateTimeout = queueConfig.idleGrace) {

    // request from a containerProxy to have a new request to execute
    case Event(request: GetActivation, _) if request.action == action =>
    ...
    //  with a period of queue.idleGrace duration a timeout is raised
    case Event( StateTimeout, data: RunningData ) =>
      logging.info(this, "RAISED AL")
      //  we have to discriminate from real idle and imposed idle. If a queue has received requests but they have been blocked
      //  it is incorrect to assume that the queue is idle
      if (queue.isEmpty && (containers.size + creationIds.size + supervisor.map{ _.activationsRecentlyRejected()}.getOrElse(0)) <= 0) {
        logging.info(
          this,
          s"[$invocationNamespace:$action:$stateName] No activations coming in ${queueConfig.idleGrace}")
        actorSystem.stop(data.schedulerActor)
        ...
        goto(TrackedIdle) using NoActors()
      } else {
        logging.info(
          this,
          s"[$invocationNamespace:$action:$stateName] The queue is timed out but there are still ${queue.size} activation messages or (running: ${containers.size} -> ${containers.toString}, in-progress: ${creationIds.size} -> ${creationIds.toString}) containers")
    ...
    case Event(FailedCreationJob(CreationId.void, _, _, _, _, _), RunningData(schedulerActor, droppingActor)) =>
      logging.info(this, "RAISED AM")
      goto(Flushing) using FlushingData(schedulerActor, droppingActor, null, null)

    case Event(FailedCreationJob, _) =>
      logging.info(this, "RAISED AN")
      stay



    case Event(GracefulShutdown, data: RunningData) =>
      logging.info(this, "RAISED AO")
      logging.info(this, s"[$invocationNamespace:$action:$stateName] The queue is going to be removed")
      actorSystem.stop(data.schedulerActor)
      actorSystem.stop(data.droppingActor)
      cleanUpDataAndGotoRemoved()
  ...
  //  state similar to the Idle but allowing the control of the incoming Activations/Containers by a dedicated component
  when(TrackedIdle, stateTimeout = queueConfig.stopGrace) {
    ...
    case Event(msg: ActivationMessage, _: NoActors) =>
      ...
      if (supervisor.exists(_.handleActivation(msg, containers.size, creationIds.size))) {
        ...
        val (schedulerActor, droppingActor) = startMonitoring()
        handleActivationMessage(msg)
        goto(TrackedRunning) using RunningData(schedulerActor, droppingActor)

      } else {

        logging.info(this, "Request rejected")
        completeErrorActivation(msg, tooManyConcurrentRequests, isWhiskError = queueConfig.failThrottleAsWhiskError)
        stay
    ...
    case Event(StateTimeout, _: NoActors) =>
      ...
      if( supervisor.map{ _.activationsRecentlyRejected()}.getOrElse{0} <= 0 ) {
        logging.info(this, s"[$invocationNamespace:$action:$stateName] The queue is timed out, stop the queue.")
        cleanUpDataAndGotoRemoved()
      }else
        stay
  ...
  whenUnhandled {
    ...
    //  event added to switch to TrackedRunning independently from the state in which we are(idle or running)

    case Event(EnableTrackedRun, data ) =>
      logging.info(this, "RAISED AU")
      goto(TrackedRunning) using data
    ...
    case Event(msg: DecisionResults, _) =>
      ...
      result match {
        ...
        case EnableTrackedRun(qsv:QueueSupervisor) =>
          if( num > 0 )
            actionRetentionTimeout = num
          logging.info(this, "RAISED-RECEIVED")
          supervisor.synchronized{
