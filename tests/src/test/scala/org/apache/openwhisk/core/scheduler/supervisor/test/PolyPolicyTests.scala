...
with StreamLogging  {
  ...
  it should "Manage minWorkers, maxWorkers, readyWorkers equal to 0" in{
    ...
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 0, 0, 0, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
  ...
  it should "Manage minWorkers, maxWorkers, readyWorkers equal to 1" in{
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 1, 1, 1, "Poly", 0,2 , "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    val parameters: (Set[String], Int, Int, Set[String]) = createEnv(0, 0, 0, 0, 1, 0, supervisor)
  ...
  it should "Manage minWorkers, maxWorkers, readyWorkers equal to n" in {
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 5, 5, 5, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    val parameters: (Set[String], Int, Int, Set[String]) = createEnv(0, 0, 0, 0, 1, 0, supervisor)
    ...
    supervisor.elaborate(parameters._1, parameters._2, parameters._3, parameters._4, None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.elaborate(parameters._1, parameters._2, parameters._3, parameters._4, None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A"), parameters._2, parameters._3, Set("A"), None) shouldBe DecisionResults(AddContainer, 4)

    //  like in the previous test, reached the required condition it will be stable
    supervisor.elaborate(Set("A","B","C"), parameters._2, parameters._3, Set("A","B","C","D"), None) shouldBe DecisionResults(Skip, 0)
  ...
  it should "Manage containers using only maxWorkers" in{
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 10, 0, 0, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    ...
    supervisor.elaborate(parameters._1, parameters._2, parameters._3 + 2, parameters._4, None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A"), parameters._2, parameters._3 + 2, Set("A"), None) shouldBe DecisionResults(AddContainer, 4)
    supervisor.elaborate(Set("A","B","C","D","E"), parameters._2, parameters._3 + 2, Set("A","B","C","D","E"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A","B","C","D","E"), parameters._2, parameters._3+4, Set("A","B","C","D","E"), None) shouldBe DecisionResults(Skip, 0)

    //  there are too many containers(0 containers are required), should drop all the containers in two iterations(4,1)
    supervisor.elaborate(Set("A","B","C","D","E"), parameters._2, parameters._3, Set("A","B","C","D","E"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("E","A","B","C")), 0)
    supervisor.elaborate(Set("E"), parameters._2, parameters._3+1, Set("E"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("E"), parameters._2, parameters._3, Set("E"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("E")), 0)
    supervisor.clean()
  }

  it should "Respect the ready containers threshold" in {
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 10, 0, 5, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    val parameters: (Set[String], Int, Int, Set[String]) = createEnv(0, 0, 0, 0, 0, 0, supervisor)
    supervisor.elaborate(parameters._1, parameters._2, parameters._3, parameters._4, None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.elaborate(Set("A"), parameters._2, parameters._3 + 1, Set("A"), None) shouldBe DecisionResults(AddContainer, 4)
    ...
    supervisor.elaborate(Set("A", "B", "C", "D", "E"), parameters._2, parameters._3+1, Set("A", "B", "C", "D", "E"), None) shouldBe DecisionResults(AddContainer, 5)
    supervisor.elaborate(Set("A", "B", "C", "D", "E", "F", "G", "H", "I", "L"), parameters._2, parameters._3+2, Set("A", "B", "C", "D", "E", "F", "G", "H", "I", "L"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A", "B", "C", "D", "E", "F", "G", "H", "I", "L"), parameters._2 , parameters._3+1, Set("A", "B", "C", "D", "E", "F", "G", "H", "I", "L"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A", "B", "C", "D", "E", "F", "G", "H", "I", "L"), parameters._2, parameters._3, Set("A", "B", "C", "D", "E", "F", "G", "H", "I", "L"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("E", "F", "A", "I", "G")), 0)
    supervisor.elaborate(Set("A", "B", "C", "D", "E"), parameters._2, parameters._3, Set("B", "C", "D", "H", "L"), None) shouldBe DecisionResults(Skip, 0)
  }

  it should "Respect the minimum containers threshold" in {
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 6, 2, 0, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    val parameters: (Set[String], Int, Int, Set[String]) = createEnv(0, 0, 0, 0, 0, 0, supervisor)
    supervisor.elaborate(parameters._1, parameters._2, parameters._3, parameters._4, None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.elaborate(Set("A"), parameters._2, parameters._3 + 1, Set("A"), None) shouldBe DecisionResults(AddContainer, 4)
    supervisor.elaborate(Set("A", "B", "C", "D", "E"), parameters._2, parameters._3 + 5, Set("A", "B", "C", "D", "E"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A", "B", "C", "D", "E"), parameters._2, parameters._3 + 6, Set("A", "B", "C", "D", "E"), None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.elaborate(Set("A", "B", "C", "D", "E", "F"), parameters._2, parameters._3 + 6, Set("A", "B", "C", "D", "E", "F"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A", "B", "C", "D", "E", "F"), parameters._2, parameters._3 + 7, Set("A", "B", "C", "D", "E", "F"), None) shouldBe DecisionResults(Skip, 0)

    //  should only grant minWorkers, doesn't interest if they are used or not
    supervisor.elaborate(Set("A", "B", "C", "D", "E", "F"), parameters._2, parameters._3 + 5, Set("A", "B", "C", "D", "E", "F"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("E")), 0)
    supervisor.elaborate(Set("A", "B", "C", "D", "E", "F"), parameters._2, parameters._3, Set("A", "B", "C", "D", "E", "F"), None) shouldBe DecisionResults(Skip, 0)
  }

  it should "Manage containers using using maxWorkers and minWorkers" in {
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 4, 2, 0, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    val parameters: (Set[String], Int, Int, Set[String]) = createEnv(0, 0, 0, 0, 0, 0, supervisor)
    ...
    supervisor.elaborate(parameters._1, parameters._2, parameters._3, parameters._4, None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.elaborate(parameters._1, parameters._2, parameters._3 + 1, parameters._4, None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A"), parameters._2, parameters._3, Set("A", "B","C","D"), None) shouldBe DecisionResults(AddContainer, 3)
    supervisor.elaborate(Set("A", "B","C","D"), parameters._2, parameters._3, Set("A", "B","C","D"), None) shouldBe DecisionResults(Skip, 0)
  ...
  it should "Manage containers using using maxWorkers and readyWorkers" in {
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 7, 0, 2, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    val parameters: (Set[String], Int, Int, Set[String]) = createEnv(0, 0, 0, 0, 0, 0, supervisor)
    supervisor.elaborate(parameters._1, parameters._2, parameters._3, parameters._4, None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.elaborate(Set( "A"), parameters._2, parameters._3, Set( "A"), None) shouldBe DecisionResults(AddContainer, 4)
    ...
    supervisor.elaborate(Set("A", "B", "C", "D","E"), parameters._2, parameters._3 + 4, Set("A", "B", "C", "D","E"), None) shouldBe DecisionResults(AddContainer,2)
    supervisor.elaborate(Set( "A", "B", "C", "D","E","F","G"), parameters._2, parameters._3, Set( "A", "B", "C", "D","E"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("E","A")), 0)
    supervisor.elaborate(Set("B", "C", "D","F","G"), parameters._2, parameters._3, Set("B", "C", "D","F","G"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.clean()
  }

  it should "Manage containers using using maxWorkers minWorkers and readyWorkers in every possible combination" in {
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 8, 3, 2, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    val parameters: (Set[String], Int, Int, Set[String]) = createEnv(0, 0, 0, 0, 0, 0, supervisor)
    supervisor.elaborate(parameters._1, parameters._2, parameters._3, parameters._4, None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.elaborate(parameters._1, parameters._2, parameters._3, parameters._4, None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A"), parameters._2, parameters._3, Set("A"), None) shouldBe DecisionResults(AddContainer, 4)
    supervisor.elaborate(Set("A","B","C","D","E"), parameters._2, parameters._3+3, Set("A","B","C","D","E"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A","B","C","D","E"), parameters._2, parameters._3+4 , Set("A","B","C","D","E"), None) shouldBe DecisionResults(AddContainer, 3)
    supervisor.elaborate(Set("A","B","C","D","E","F","G","H"), parameters._2, parameters._3+4 , Set("A","B","C","D","E","F","G","H"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A","B","C","D","E","F","G","H"), parameters._2, parameters._3+6 , Set("A","B","C","D","E","F","G","H"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.elaborate(Set("A","B","C","D","E","F","G","H"), parameters._2, parameters._3+3 , Set("A","B","C","D","E","F","G","H"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("E","F","A")), 0)
    supervisor.elaborate(Set("B","C","D","G","H"), parameters._2, parameters._3 , Set("B","C","D","G","H"), None) shouldBe DecisionResults(Skip, 0)
    supervisor.clean()
  }

  it should "React to a dynamic change of the parameters" in{
    val stateRegistry: StateRegistry = new MockStateRegistry(namespace, action)
    val config = SchedulingSupervisorConfig(enableSupervisor = true, 1, 1, 0, "Poly", 0, 2, "AcceptAll", 2, 500, "Consolidation")
    val supervisor: QueueSupervisor = new QueueSupervisor(namespace, action, config, Parameters.apply(), stateRegistry, etcdClient)
    val parameters: (Set[String], Int, Int, Set[String]) = createEnv(0, 0, 0, 0, 0, 0, supervisor)
    ...
    supervisor.elaborate(Set("A","B"), parameters._2, 3, Set("A","B"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("A")), 0)
    ...
    supervisor.elaborate(Set("C"), parameters._2, 0, Set("C"), None) shouldBe DecisionResults(AddContainer, 3)
    supervisor.setReadyWorkers(3)
    supervisor.elaborate(Set("C","D","E","F"), parameters._2, 1, Set("C","D","E","F"), None) shouldBe DecisionResults(Skip, 0)
    ...
    supervisor.elaborate(Set("C","D","E","F"), parameters._2, 3, Set("C","D","E","F"), None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.setMinWorkers(0)
    ...
    supervisor.elaborate(Set("A","B","C","D","E"), parameters._2, 1, Set("A","B","C","D","E"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("E","A","B","C")), 0)
    supervisor.elaborate(Set("D"), parameters._2, 0, Set("D"), None) shouldBe DecisionResults(RemoveReadyContainer(Set("D")), 0)
    supervisor.elaborate(parameters._1, parameters._2, 0, parameters._4, None) shouldBe DecisionResults(Skip, 0)
    ...
    supervisor.elaborate(Set("A"), parameters._2, 0, Set("A"), None) shouldBe DecisionResults(AddContainer, 1)
    supervisor.setMaxWorkers(4)
    supervisor.setMinWorkers(3)
